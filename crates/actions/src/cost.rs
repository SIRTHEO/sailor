...
printf '{"result":"la risposta vera","model":"modello-di-prova","total_cost_usd":0.5,"usage":{"input_tokens":1000000,"output_tokens":1000000,"cache_read_input_tokens":1000000}}'"#;
    ...
    /// A resolver whose engine can be told the most one call may spend. It is
    /// where, in real life, a descriptor's `native_spend_cap` arrives.
    struct HoldsToACeiling(Declares);

    impl ToolResolver for HoldsToACeiling {
        fn resolve(&self, id: &str) -> Result<String, String> {
            self.0.resolve(id)
        }
        ...
        fn spend_ceiling_option(&self, _id: &str) -> Option<crate::reserve::CeilingOption> {
            Some(crate::reserve::CeilingOption {
                args: vec!["--max-budget-usd".to_owned()],
                unit: crate::reserve::UNIT_CURRENCY.to_owned(),
            })
        }
    }

    /// A cheap call that writes down the line it was invoked with.
    const A_SMALL_CALL: &str = r#"cat > /dev/null
printf '%s\n' "$@" > "$(dirname "$0")/argv"
printf '{"result":"fatto","model":"modello-di-prova","usage":{"input_tokens":100000,"output_tokens":0,"cache_read_input_tokens":0}}'"#;

    /// The shared state as the executor prepares it under a capped run.
    fn capped(run: &str, step: &str, cap_micros: i64) -> SharedState {
        let mut state = shared(run, step);
        state.insert(flow::CURRENT_CAP.to_owned(), json!(cap_micros));
        state
    }

    /// **THE CONTROL BEFORE THE CALL, NOT AFTER THE SPEND.** A run capped at
    /// 1.00 whose every call is held to 0.60: two calls at 0.30 fit, and the
    /// third is refused because 0.60 does not fit the 0.40 left — while the
    /// older rule, «stop once the spend reaches the cap», would have started
    /// it, 0.60 being under 1.00. The refusal says both numbers.
    #[test]
    fn a_call_whose_reserve_does_not_fit_the_remainder_never_starts() {
        let dir = scratch("ammissione");
        let price_list = write_price_list(&dir);
        let bin = fake_engine(&dir, "motore", A_SMALL_CALL);
        let ledger = Ledger::open(dir.join("deposito")).expect("aprire il deposito");
        let action = ExternalEngineAction::resolving_with(HoldsToACeiling(Declares {
            bin,
            recipe: Some(declaring_recipe()),
        }))
        .recording_to(Some(ledger))
        ...
        with_price_list(Some(&price_list), || {
            action.execute(&input, &capped("corsa-al-tetto", "passo-1", 1_000_000))
        })
        .expect("the first call fits the remainder");
        assert!(
            argv_of(&dir).contains(&"0.600000".to_owned()),
            "the ceiling reserved is the ceiling imposed: {:?}",
            argv_of(&dir)
        );
        with_price_list(Some(&price_list), || {
            action.execute(&input, &capped("corsa-al-tetto", "passo-2", 1_000_000))
        })
        .expect("the second still fits");

        let refused = with_price_list(Some(&price_list), || {
            action.execute(&input, &capped("corsa-al-tetto", "passo-3", 1_000_000))
        })
        ...
        assert!(
            ...
            "the two numbers are both there: {}",
            refused.said
        );
        assert_eq!(
    ...
    /// The other half: with nothing to reserve against, the same run is not
    /// suspended before the remainder is gone — and when it is, the refusal
    /// says the reserve could not be known instead of showing a figure.
    #[test]
    fn a_call_nobody_can_reserve_says_so_instead_of_showing_a_number() {
        let dir = scratch("senza-riserva");
        let price_list = write_price_list(&dir);
        let bin = fake_engine(&dir, "motore", A_SMALL_CALL);
        let ledger = Ledger::open(dir.join("deposito")).expect("aprire il deposito");
        let action = ExternalEngineAction::resolving_with(Declares {
        ...
        // nothing left, which is the only moment an unreservable cap bites.
        for step in ["passo-1", "passo-2", "passo-3"] {
            with_price_list(Some(&price_list), || {
                action.execute(&input, &capped("corsa-senza-riserva", step, 900_000))
            })
            .expect("under an unreservable cap the older rule still applies");
        }
        let refused = with_price_list(Some(&price_list), || {
            action.execute(&input, &capped("corsa-senza-riserva", "passo-4", 900_000))
        })
        ...
        assert!(
            refused.said.contains("stop threshold")
                && refused.said.contains(crate::reserve::NATIVE_SPEND_CAP),
