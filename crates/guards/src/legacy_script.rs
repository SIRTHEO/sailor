...
mod tests {
    ...
    fn a_deciding_script_inside_the_config_is_refused() {
        assert!(denied("/home/someone/.claude/scripts/deposito-guasti.sh", true));
        assert!(denied("/home/someone/.claude/scripts/queue-patrol.sh", true));
        assert!(denied(
            "/home/someone/.claude/skills/hooks/lingua-codice.py",
            true
        ));
    }
    ...
    fn a_brand_new_script_says_something_different_from_an_edit() {
        let Decision::Deny(nuovo) = judge("/home/someone/.claude/scripts/x.py", false, false) else {
            panic!("doveva negare il file nuovo");
        };
        let Decision::Deny(esistente) = judge("/home/someone/.claude/scripts/x.py", true, false) else {
    ...
    fn the_second_gesture_is_shorter_but_still_refused() {
        let path = "/home/someone/.claude/scripts/deposito-guasti.sh";
    ...
    fn the_four_legitimate_shapes_pass_in_silence() {
        for path in [
            ...
            "/home/someone/.claude/guard-scope.sh",
            "/home/someone/.claude/scripts/deposito-guasti.test.sh",
            "/home/someone/.claude/scripts/queue-select.test.sh",
            "/home/someone/.claude/scripts/gesti-theo-2026-08-24.sh",
            "/home/someone/.claude/scripts/apply-theo-decisions.sh",
            "/home/someone/.claude/plugins/qualcosa/setup.sh",
            "/home/someone/.claude/workflows/divide-and-verify.js",
        ] {
            assert!(
    ...
    fn scripts_outside_the_config_are_none_of_our_business() {
        for path in [
            "/home/someone/other-repo/work/scripts/deploy.sh",
            "/home/someone/orca/general/tools/x.py",
            "/tmp/prova.sh",
        ] {
            assert!(
                matches!(judge(path, true, false), Decision::Pass),
    ...
    fn rust_and_prose_are_never_touched() {
        for path in [
            "/home/someone/.claude/rust/crates/guards/src/legacy_script.rs",
            "/home/someone/.claude/docs/libro-di-bordo.md",
