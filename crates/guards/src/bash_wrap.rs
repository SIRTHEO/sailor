...
/// un'opzione**.
///
/// Il salto dell'argomento non è pedanteria: `git -C /home/someone/.claude log`
/// è la forma normale in questa casa, e senza questo il sottocomando risultava
/// `/home/someone/.claude` — un `git log` su cinque non veniva riconosciuto.
...
mod tests {
    ...
    fn the_listed_families_are_recognised_inside_a_chain() {
        for command in [
            "cargo test -p guards",
            "cd /home/someone/.claude/rust && cargo test -p guards 2>&1 | tail -40",
            "grep -rn 'foo' /home/someone/orca --include='*.ts'",
            "git -C /home/someone/.claude log --oneline -30",
            "find /home/someone/.claude -name '*.rs'",
            "pnpm --dir /home/someone/other-repo/work -r test",
            "npx vitest run",
            "ls -R /home/someone/orca/workspaces",
        ] {
    ...
    fn anything_outside_the_list_is_left_alone() {
        for command in [
            "echo ciao",
            "cat /home/someone/.claude/settings.json",
            "sed -n '1,40p' file.rs",
            "python3 misura.py",
            "gh pr view 479",
            "orca worktree list",
    ...
    fn an_option_value_is_not_mistaken_for_the_subcommand() {
        assert_eq!(
            subcommand_of("-C /home/someone/.claude log --oneline".split_whitespace()),
            Some("log")
        );
        assert_eq!(
        ...
        assert_eq!(
            subcommand_of("--dir /home/someone/other-repo/work -r test".split_whitespace()),
            Some("test")
        );
        assert_eq!(subcommand_of("--version".split_whitespace()), None);
        // E il caso opposto resta fermo: `git -C <dir> status` non è in lista.
        assert!(!should_wrap("git -C /home/someone/.claude status --short"));
    }
