...
fn roots() -> &'static [(&'static str, Regex)] {
    ...
    RE.get_or_init(|| {
        vec![
            (
                "suite",
                Regex::new(r"/other-repo/work/suite\b|workspaces/suite/").unwrap(),
            ),
            (
                "a-service",
                Regex::new(r"/other-repo/work/a-service\b|workspaces/a-service/").unwrap(),
            ),
            (
                "a-client",
                Regex::new(r"/other-repo/work/a-client\b|workspaces/a-client/").unwrap(),
            ),
            ("packages", Regex::new(r"/other-repo/work/packages\b").unwrap()),
            ("dev-stack", Regex::new(r"/other-repo/work/dev-stack\b").unwrap()),
            (
                "personale",
...
mod tests {
    ...
    fn i_sette_riconoscitori_dell_originale() {
        assert_eq!(
            aree(json!({"file_path": "/home/someone/other-repo/work/suite/src/x.ts"})),
            set(&["suite"])
        );
        assert_eq!(
            aree(json!({"command": "pnpm --dir /home/someone/other-repo/work/a-service run test"})),
            set(&["a-service"])
        );
        assert_eq!(
            aree(json!({"file_path": "/home/someone/other-repo/work/a-client/src/a.ts"})),
            set(&["a-client"])
        );
        assert_eq!(
            aree(json!({"file_path": "/home/someone/other-repo/work/packages/geo/x.ts"})),
            set(&["packages"])
        );
        assert_eq!(
            aree(json!({"command": "docker compose -f /home/someone/other-repo/work/dev-stack/x.yml up"})),
            set(&["dev-stack"])
        );
        assert_eq!(
            aree(json!({"file_path": "/home/someone/.claude/rules/x.md"})),
            set(&["configurazione"])
        );
        assert_eq!(
            aree(json!({"file_path": "/home/someone/personal/sailor/x.ts"})),
            set(&["personale"])
        );
        assert_eq!(
            aree(json!({"command": "ls /home/someone/orca/general"})),
            set(&["personale"])
        );
    }
    ...
    fn i_trascritti_e_la_cronologia_non_sono_configurazione() {
        assert_eq!(
            aree(json!({"file_path": "/home/someone/.claude/projects/x/y.jsonl"})),
            set(&[])
        );
        assert_eq!(
            aree(json!({"file_path": "/home/someone/.claude/history/x"})),
            set(&[])
        );
        ...
        // com'era.
        assert!(!is_configuration("/home/someone/.claude/projectsX/y"));
    ...
    fn un_worktree_orca_conta_come_il_suo_repo() {
        ...
        assert_eq!(
            aree(json!({"command": "ls /home/someone/orca/workspaces/packages/wt"})),
            set(&[])
        );
    }
    ...
    fn il_confine_di_parola_distingue_un_repo_da_uno_che_gli_somiglia() {
        ...
        assert_eq!(
            aree(json!({"file_path": "/home/someone/other-repo/work/suiteXX/x.ts"})),
            set(&[])
        );
        assert_eq!(
            aree(json!({"file_path": "/home/someone/other-repo/work/suite-vecchia/x.ts"})),
            set(&["suite"])
        );
        assert_eq!(
            aree(json!({"file_path": "/home/someone/other-repo/work/suite"})),
            set(&["suite"])
        );
    }
    ...
    fn due_aree_in_un_comando_solo() {
        assert_eq!(
            aree(json!({"command":
                "diff /home/someone/other-repo/work/suite/a /home/someone/other-repo/work/packages/b"})),
            set(&["packages", "suite"])
        );
    }
    ...
    fn i_campi_si_leggono_tutti_e_solo_se_sono_stringhe() {
        ...
        assert_eq!(
            aree(json!({"path": "/home/someone/other-repo/work/suite/x"})),
            set(&["suite"])
        );
        assert_eq!(
            aree(json!({"notebook_path": "/home/someone/other-repo/work/suite/x.ipynb"})),
            set(&["suite"])
        );
