...
/// censimento delle rotte del a-service — due regex su `method:` e
...
mod tests {
    ...
    fn a_question_asked_inside_the_transcripts_is_not_sent_to_the_index() {
        for dove in [
            ...
            "/home/someone/.claude/projects/-Users-theo-orca-general/x.jsonl",
            "/home/someone/.claude/state/staffetta.log",
        ] {
        ...
        for dove in [
            "/home/someone/.claude/rust/crates/guards/src",
            "/home/someone/.claude/scripts/queue-patrol.sh",
            "/home/someone/.claude/docs",
        ] {
            assert!(!is_out_of_perimeter(dove), "{dove}");
        }
    }
    ...
    fn the_search_trace_is_worth_twenty_minutes() {
        ...
        std::fs::write(
            ws.home.join(".claude").join("state").join("socraticode-progetti.txt"),
            "/home/someone/repo-dichiarato-per-prova\n",
        )
        .unwrap();

        let write_input = |session: &str| hook_io::HookInput {
            session_id: Some(session.to_string()),
            cwd: Some("/home/someone/repo-dichiarato-per-prova".to_string()),
            tool_name: Some("Write".to_string()),
            tool_input: Some(serde_json::json!({
                "file_path": "/home/someone/repo-dichiarato-per-prova/src/nuovo.ts",
            })),
            ..Default::default()
        };
    ...
    fn a_bash_search_is_judged_where_it_points_not_where_it_runs() {
        ...
        let verdict = judge(
            ...
            &bash_input(
                &altrove,
                "grep -rn \"dove si gestisce il pagamento fallito\" \
                 /home/someone/repo-dichiarato-per-prova/src",
            ),
        );
        assert!(
            matches!(verdict.decision, Decision::Block(_)),
    ...
    fn a_throwaway_file_inside_a_declared_root_stays_out_of_the_reuse_case() {
        ...
        let write_input = |path: &str| hook_io::HookInput {
            ...
            cwd: Some("/home/someone/.claude".to_string()),
            tool_name: Some("Write".to_string()),
            tool_input: Some(serde_json::json!({ "file_path": path })),
            ..Default::default()
        };
        for path in [
            "/home/someone/.claude/state/nuovo.ts",
            "/home/someone/.claude/scratchpad/nuovo.ts",
        ] {
            assert!(
        ...
        assert!(matches!(
            judge(&ws, &write_input("/home/someone/.claude/rust/nuovo.ts")).decision,
            Decision::Block(_)
        ));
    }
