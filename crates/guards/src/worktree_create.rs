...
//! sta nei repo Gyver e se il comando non li nomina. Provato il 18/08/2026
//! sull'esemplare vero, quello dei sei commit: **passa**, perché è su `sailor`.
//! Dentro il perimetro Gyver i due si sovrappongono e negano entrambi. La
...
mod tests {
    ...
    fn a_persistent_tree_inside_the_managed_root_is_blocked() {
        assert!(blocks(
            "git worktree add /Users/theo/orca/workspaces/suite/nuova-copia"
        ));
    }
    ...
    fn a_mktemp_tree_is_left_alone() {
        assert!(!blocks(
            "cd /Users/theo/personal/sailor && W=$(mktemp -d)/mut && git worktree add -q $W HEAD"
        ));
    }
    ...
    fn the_branch_flag_is_not_mistaken_for_the_path() {
        ...
        let decision = judge(
            "git worktree add -b work/nuova /Users/theo/orca/workspaces/packages/nuova develop",
        );
    ...
    fn flags_without_a_value_do_not_shift_the_path() {
        assert!(blocks(
            "git worktree add -q --detach /Users/theo/orca/workspaces/whatsapp/x"
        ));
    }
    ...
    fn the_reason_flag_does_not_become_the_branch() {
        let Decision::Block(message) =
            judge("git worktree add --reason parking /Users/theo/orca/workspaces/suite/x")
        else {
            panic!("this must block");
        };
    ...
    fn a_worktree_command_that_is_not_add_passes() {
        ...
        assert!(!blocks(
            "git worktree remove /Users/theo/orca/workspaces/suite/vecchia"
        ));
    }
    ...
    fn the_managed_root_needs_both_repo_and_name() {
        ...
        assert!(!blocks("git worktree add /Users/theo/orca/workspaces/suite"));
    }
    ...
    fn a_quoted_command_inside_a_string_is_not_a_call() {
        ...
        assert!(!blocks(
            "echo 'git worktree add /Users/theo/orca/workspaces/suite/x'"
        ));
    }
