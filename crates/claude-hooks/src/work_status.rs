...
/// `~/gyver/work` contiene `~/gyver/work/suite`, e la lavorazione è la seconda.
...
///
/// «Richiesta unita» non basta per dire completata: `whatsapp/media-link-recovery`
...
mod tests {
    ...
    fn lo_slug_esce_dall_url_stampato() {
        assert_eq!(
            slug_of_pull(Some(&json!({"stdout": "https://github.com/Gyver-work/packages/pull/32"}))),
            "gyver-work/packages"
        );
        assert_eq!(
            slug_of_pull(Some(&json!("git@github.com:Gyver-work/packages/pull/9"))),
            "gyver-work/packages"
        );
