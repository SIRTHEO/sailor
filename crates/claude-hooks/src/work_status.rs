...
mod tests {
    ...
    fn le_opzioni_globali_stanno_in_mezzo() {
        for c in [
            "gh -R Other-repo-Work/a-service pr merge 530 --squash",
            "gh --repo Other-repo-Work/suite pr merge 77 --merge",
            "gh -R=Other-repo-Work/suite pr merge 77",
        ] {
            assert!(merges_request(c), "non riconosciuto: {c}");
        }
    ...
    fn il_numero_e_il_repo_si_leggono_dal_comando() {
        for (c, number, slug) in [
            (
                "gh -R Other-repo-Work/a-service pr merge 530 --squash",
                "530",
                "other-repo-work/a-service",
            ),
            ("gh pr merge --squash 468", "468", ""),
            ("cd /a/x && gh pr merge 12 --merge", "12", ""),
            (
                "gh pr merge https://github.com/Other-repo-Work/suite/pull/77 --squash",
                "77",
                "other-repo-work/suite",
            ),
    ...
    fn la_copia_si_sceglie_dal_ramo_della_richiesta() {
        ...
        assert!(copies_of_branch(&worktrees, "other-repo-work/suite", "mio-ramo").is_empty());
    ...
    fn il_remoto_passa_da_un_host_di_comodo() {
        ...
        assert_eq!(
            slug("git@github.com-work:Other-repo-work/a-service.git"),
            "other-repo-work/a-service"
        );
        assert_eq!(
        ...
        assert_eq!(slug("git@github.com:Other-repo-Work/packages.git"), "other-repo-work/packages");
        assert_eq!(slug("https://github.com/Other-repo-Work/suite"), "other-repo-work/suite");
