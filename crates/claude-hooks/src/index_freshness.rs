...
/// il 20/08/2026, `origin/HEAD` risolve a `staging` per a-client e
/// a-service, a `main` per packages, e solo per suite a `develop`.
...
/// che è una sottocartella di un repo (`other-repo/work/.claude` è dentro il repo
/// `other-repo/work`) alla stessa radice, cosicché due radici dichiarate per lo
