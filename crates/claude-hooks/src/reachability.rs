...
//!
//! Porta di `gyver/work/.claude/scripts/reachability.py`, scritto il 18/08/2026
...
fn workspace() -> PathBuf {
    env_path("REACH_WORKSPACE", PathBuf::from("/Users/theo/gyver/work"))
}
...
/// `work.gyver.motore` è `disabled` dal 16/08, e nessuno di quei nomi somiglia a
...
mod tests {
    ...
    fn reads_a_loaded_label_with_no_pid() {
        ...
        assert_eq!(
            ...
            set(&["com.a.running", "work.gyver.idle"])
        );
    ...
    fn reads_the_disabled_ones() {
        assert_eq!(
            disabled_labels("\"work.gyver.motore\" => disabled\n\"other\" => enabled"),
            set(&["work.gyver.motore"])
        );
    ...
    fn plist_label_comes_from_the_field() {
        assert_eq!(
            plist_label("<key>Label</key><string>work.gyver.x</string>"),
            "work.gyver.x"
        );
    ...
    fn a_root_is_what_fires_not_what_exists() {
        ...
        std::fs::write(agents.join("vivo.plist"),
                       "<key>Label</key><string>work.gyver.vivo</string>").unwrap();
        std::fs::write(agents.join("fermo.plist"),
                       "<key>Label</key><string>work.gyver.fermo</string>").unwrap();
        ...
        let loaded = set(&["work.gyver.vivo"]);
