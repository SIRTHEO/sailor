...
mod tests {
    ...
    fn recognises_a_memory_file_and_rejects_everything_else() {
        assert!(is_memory_file(Path::new(
            "/Users/theo/.claude/projects/-Users-theo-orca-general/memory/x.md"
        )));
        assert!(!is_memory_file(Path::new(
            "/Users/theo/orca/general/src/a.rs"
        )));
        assert!(!is_memory_file(Path::new(
            "/Users/theo/.claude/projects/-x/memory/x.txt"
        )));
        // Una cartella "memory" fuori da "projects" non e' quella che cerchiamo.
        assert!(!is_memory_file(Path::new(
            "/Users/theo/.claude/skills/x/memory/x.md"
        )));
    }
