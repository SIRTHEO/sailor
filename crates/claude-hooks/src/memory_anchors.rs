...
fn roots() -> Vec<PathBuf> {
    ...
    let umbrellas = [h.join("other-repo/work"), h.join("personal")];
    let mut out = vec![
        h.join(".claude"),
        h.join(".claude/rust"),
...
fn build_remote_index() -> BTreeMap<String, Vec<String>> {
    ...
    for root in roots() {
        ...
        // `develop` e' il ramo d'integrazione dei repo Other-repo, `main` quello
            ...
            while let Some(dir) = stack.pop() {
                ...
                for e in entries.flatten() {
                    ...
                    if p.is_dir() {
                        ...
                        // `other-repo/work/.claude/scripts/`) e i tre `ci.yml` —
...
/// `/home/someone/orca/general` diventa `-Users-theo-orca-general`. E' la
...
mod tests {
    ...
    fn the_project_folder_comes_from_the_working_directory() {
        let dir =
            project_memory_dir(r#"{"session_id":"x","cwd":"/home/someone/orca/general"}"#).unwrap();
        assert!(
