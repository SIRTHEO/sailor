            ...
            while let Some(dir) = stack.pop() {
                ...
                for e in entries.flatten() {
                    ...
                    if p.is_dir() {
                        ...
                        // `plancia.py` mentre sta in `other-repo/work/.claude/scripts/`.
...
mod tests {
    ...
    fn the_project_folder_comes_from_the_working_directory() {
        let dir = project_memory_dir(r#"{"session_id":"x","cwd":"/home/someone/orca/general"}"#)
            .unwrap();
        assert!(
