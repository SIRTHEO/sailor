...
/// A HOOK RUNS INSIDE THE SESSION, so which profile it is under is read from
/// the session's own variable first, never guessed from the store's
/// separately switchable "active" profile — the two can disagree the moment a
/// session is launched under an explicit `CLAUDE_CONFIG_DIR` (or the
/// equivalent for another command line) while somebody switched the active
/// one afterwards, and until now this file always trusted the switch.
...
/// [`profile_home_of`], with the session's own variable already read: pure,
/// so a test can say what a session was launched under without touching this
