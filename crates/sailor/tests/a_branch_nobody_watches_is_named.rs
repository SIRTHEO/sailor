...
fn a_branch_the_trunk_already_holds_is_not_adrift() {
    ...
    assert!(adrift(&standing).is_empty());
}
...
fn the_trunk_is_never_reported() {
    let standing = vec![branch("main", false, false, LONG_ENOUGH * 10)];
    assert!(adrift(&standing).is_empty());
}
...
fn the_report_leads_with_the_one_left_longest() {
    ...
    let names: Vec<&str> = adrift(&standing).iter().map(|b| b.name.as_str()).collect();
