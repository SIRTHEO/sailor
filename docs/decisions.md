...
## ADR-025: An entry point is a sensor the flow declares

**Status:** accepted

**Context.** A flow could start from a person (`manual`), from a schedule, or from a session event, and the session event is the only real event the product listens to. The next ones asked for are all of one family: a git ref moved, a file changed, a count in the store crossed a threshold. Built one by one, each is a new source kind, a new listener and a new branch in the beat, and the product grows a hard-coded event for every case somebody meets — which is ADR-001's engine list and ADR-020's forge list again, for events. Every one of those cases is the same question: read something, and start the flow if it is not what it was.

**Decision.** A flow's trigger step may declare `"source": "sensor"` with a `sensor` object — `action`, `with`, an optional JSON `pointer` into the output, and an optional `cooldown_secs` (300 when absent). The sensor is one node run through the same action registry every step uses; what it reads, or the part of it at `pointer`, is the observation, and its fingerprint is the digest of that value's canonical JSON. It works where a step of its flow would: a relative `workdir` hangs off the project root, an absolute one is refused, and an absent one is offered the root when the action has a place for it. On every tick the beat runs every sensor flow's sensor and compares the fingerprint with the one kept for that flow in the store (`sensor_readings`): a first reading is kept as the baseline and starts nothing; an unchanged one does nothing; a changed one outside the cooldown since the sensor last started the flow starts it with one JSON object `{before, after, fingerprint_before, fingerprint_after}` as its text and keeps the new reading; a changed one inside the cooldown neither starts nor is kept, so it fires once the cooldown ends; a sensor that fails, times out or whose pointer reaches nothing starts nothing, keeps nothing, and the beat says so. Three guarantees hold it. **A sensor neither spends nor writes**: `sailor flow check` refuses a sensor whose action is not registered, may spend, or does not declare `only_reads` — a declaration each action makes about itself beside `may_spend`, false unless the action says otherwise — along with a malformed pointer and a negative cooldown. **State is compared, not signals counted**: two changes between ticks are one run carrying the first reading and the last, nothing is lost and nothing is replayed, so a flow started this way must be safe to run twice. **A cooldown separates two starts**, so a reading that flickers cannot start its flow at every tick.

**Consequences.** Watching a new thing is writing a flow, not a change to Sailor: the event is data in the flow, and what it may do is decided by what the action declares about itself, never by a list of action names in the check. The beat is the sensor's clock, but a flow never starts because time passed, only because the state changed. Left out: no push notification exists, so a change is seen at the next tick and not before; a reading larger than 16 KiB is kept as its head beside the full fingerprint; an action that hangs is left behind after 60 seconds rather than killed, since an action has no handle to stop it by; and a sensor is one node — a sensor made of a subgraph is a later decision.

## ADR-026: A merge on the trunk is carried by its integration

**Status:** accepted

**Context.** The trunk is reached only through a merged pull request, and `integrate-on-the-trunk` is the flow that makes that merge after a clean review, the gates on the combined tree, a person's green on the lines left to one, and the publication gate. Nothing on the forge said whether a merge had come that way. Measured over the pull requests merged since 21/09/2026: 27 of 50 have no completed run of the flow in the ledger naming their branch. The required checks could not tell them apart, because every one of them is posted on the head of a request before anybody integrates it, and `sailor/private-names` is posted by `scripts/attest-private-names.sh` alone, which a person can run by hand.

**Decision.** The trunk requires the commit status `sailor/integrated` once its ruleset names it, and only the integration flow posts it. Its step `integrated` runs once every check before the merge has passed (the gates, a person's green when a line was left to one, the publication gate, the private-names attestation and the request made ready) and posts, through `scripts/attest-integration.sh` and as the account in `sailor.forgeAs`, a `success` on the reviewed head naming the run and the commit the verdict was given on: `run <run id>; verdict on <sha12>`. The run id is the one the executor writes, handed to every shell step as `SAILOR_RUN` over whatever the flow's own `env` says. A post the forge refuses breaks the step, and the merge step waits for it and refuses to merge unless the status was posted on that same head in this run.

**Consequences.** A merge made outside the flow waits on a status nobody posted, once the forge requires it. What this does not prevent: the status is a claim made by an account, and the same account can post it by hand, so it proves that a run of this flow was asked for by name, not that one ran; the description is what a reader follows back to the ledger. The ruleset's bypass for administrators lets such an account merge without any required check, so the requirement binds only once that bypass is removed. Both are forge settings a person changes, not part of the product. **The order is release first, then the ruleset**: a ruleset that requires `sailor/integrated` before the binary in service posts it blocks every merge, the integration flow's own included. `publication boundary` does not read this status the way it reads `sailor/private-names`: that job runs when a head is pushed, long before an integration, so waiting for the status there would hold it red on every push, and a pull request can edit the workflow that would check the creator. The lock is the ruleset, and once it requires the status, the table of required checks in `docs/gates.md` owes it a row proved red.
