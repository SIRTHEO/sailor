...
**Consequences.** A flow names what it gives back in its own graph, as data, and the executor names no tool, forge or machine to do it. The step runs once per run: it is settled like any other once it closes, so a resume finds it done. A halt by hand, a wall or a spend cap still holds the front before it opens, so a step that gives back is not run past one of them; that is left for a later decision, with a measurement.

## ADR-024: An entry point is a sensor the flow declares
