...
sola** (`~/gyver/work`, `~/personal`) — `crates/inventory/src/lib.rs:648-671`.
...
| `file:riga` | Cosa c'è | Perché viola | Costo per toglierlo |
|---|---|---|---|
| `crates/inventory/src/lib.rs:652-653` | `home.join("gyver").join("work"),` / `home.join("personal"),` | **La peggiore.** Le cartelle di lavoro di una persona sola, compilate. `gyver` è il nome di un datore di lavoro; `personal` una convenzione privata. Su un'altra macchina l'inventario dei repo è vuoto e non lo dice | Rifacimento piccolo: le basi diventano un campo di configurazione (`SAILOR_WORK_ROOTS`, o una chiave nella casa) con questo elenco come predefinito **dichiarato**, non come fatto |
| `crates/inventory/src/lib.rs:658-666` | `(".agents", home.join(".agents").join("skills")),` e `("gyver/work/.agents", home.join("gyver").join("work").join(".agents").join("skills"))` | Due magazzini di questa macchina, uno con il nome del datore di lavoro dentro l'etichetta **e** dentro il percorso | Come sopra, stesso file di configurazione |
...
| `flows/chiedi-all-indice.flow.json:266-267` | `"who": "theo",` / `"where": "/Users/theo/personal/sailor"` | Un flusso spedito nel repo che si può eseguire **in un posto solo**: è la forma esatta del guasto 25 che `crates/flow/src/workspace.rs:3-8` racconta di aver chiuso | Due righe: la radice viene da chi lancia |
...
- `crates/flow/src/workspace.rs:4` — cita `"workdir": "/Users/theo/personal/sailor"`
...
**1. `crates/inventory/src/lib.rs:652-653` — `~/gyver/work` e `~/personal`.**
