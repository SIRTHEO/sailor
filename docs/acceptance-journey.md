...
| 10 | `env -u SAILOR_LEDGER sailor faults check docs/faults-encountered.md` on the page as committed at the candidate, with no render first; the one step read against this machine's own store | «agrees with the store as it stood through fault N»: the check reads the stamp from the page and nothing else. The stamp's fault number must be the highest the store held at that moment, and the whole page is compared, byte for byte, with a render of the store as it stood at its stamp, which writes every other line itself, the opening prose included. Without a store, the forge's judge of the page holds it only to the template and to itself: a row invented or removed in the shape the render writes, or a wrong «more», is seen by this check alone. The forge's judge also refuses a page with no rows under a stamp from a kept history, so the day no open fault has a public summary, an honest page is red in the forge. Faults opened or closed after the page was counted, by any binary, do not move it. The first line that differs is named and is red; «cannot tell what stood then» is red too: the page was not counted from this store's history, which step 0 is for. The walk leaves the page as committed |
