...
| 27 | 31/08 | **Il guasto 1 è tornato da un'altra porta.** Gli argomenti del blocco `usage` di un descrittore si aggiungono **dopo** quelli di `ask`: per `agy` la riga diventa `--mode plan --print --output-format json «prompt»`, e `--print` prende `--output-format` come propria domanda. Il prompt viene ignorato e il passo muore. La nota del descrittore dichiara di aver risolto esattamente questo il 28/08 mettendo `--mode` prima di `--print` — ma prevedeva solo *quei* due argomenti | Lanciando un flusso nuovo che chiedeva `agy` **per primo**. Nessun flusso esistente lo fa: tutti lo mettono secondo o terzo, quindi su questa macchina — dove `claude-code` c'è sempre — quel ramo non era mai stato eseguito. Provato A/B: `--print` prima di `--output-format` dà l'errore, dopo risponde e restituisce le chiavi che il descrittore si aspetta | Il guasto 1 era stato chiuso **senza costruire la cura che gli era stata scritta accanto** — «una prova che esegue davvero ogni riga di comando prima che finisca in un flusso». Serve quella, e per **ogni combinazione** di blocchi che compongono la riga (`ask`, `usage`, versione), non per la sola forma nominale: qui i due blocchi erano giusti separatamente e sbagliati insieme. Serve anche che una catena di motori **provi il successivo** quando il primo muore per una riga malformata, invece di fermare il passo | **aperto** |
