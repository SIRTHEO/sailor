...
| 8. Publish integration, reconcile the pull request | coordinator invokes the push and the merge | required policy-controlled steps | remote trunk carries the candidate, the host reports the pull request merged, not merely pushed; an API failure is a retryable obligation | policy file's `merge`/`push` settings, the account a tree pushes as | remote commit, observed pull-request state | 9, 11 |
