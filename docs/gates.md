...
- `cargo test -p sailor -j 1 --test a_shipped_flow_that_changes_raises_its_version` — a shipped flow whose text changed raises its `version`, so a copy of it can be told stale
