...
GIT_REMOTI = [
    {"match": ["/a/packages/x", "remote"], "stdout": "git@github.com:Gyver-Work/packages.git\n"},
    {"match": ["/a/suite/y", "remote"], "stdout": "https://github.com/Gyver-Work/suite\n"},
    {"match": ["/a/packages", "remote"], "stdout": "git@github.com:Gyver-Work/packages.git\n"},
]


def gh_dice(repo, richieste):
    return {
        "match": [f"Gyver-Work/{repo}"],
        "stdout": json.dumps(richieste),
    }


def gh_muto(repo):
    return {"match": [f"Gyver-Work/{repo}"], "rc": 1}
...
URL = "https://github.com/Gyver-work/packages/pull/32"
CREATA = json.dumps(
         ...
         {"stdin": bash(dentro, {"stdout": URL}),
          "spec": {"orca": orca_normale(LISTA_DUE),
                   "git": [{"match": ["/a/packages", "remote"],
                            "stdout": "git@github.com:Gyver-Work/suite.git\n"}]}}),
