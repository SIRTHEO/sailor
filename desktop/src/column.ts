/**
 * ONE READING ANSWERS THE WHOLE COLUMN. The column asks four questions —
 * which workspaces, which trees inside each, which flows belong nowhere in
 * particular, and what is open outside every workspace — and four calls
 * answered them at four different moments, so the column could show a tree as
 * current while its flows still belonged to the last one.
 *
 * The shape is declared here and fed from `columnStub` until the one backend
 * reading lands: `theColumn` is the seam, and only its body changes then.
 */
